...
def grab_expected_output(name):
    with open(name_to_path(name)) as fh:
        return [
            sanitize_output(x)
            for x in safe_file_iterator(fh)
...
def run_example_and_collect_output(name):
    return [
        ...
        for x in subprocess.check_output(
            ["python", name_to_path(name)], stderr=subprocess.STDOUT
        )
        .strip()
        .splitlines()
    ]
...
class ExampleFileTestSuite:
    def __init__(self, name):
        ...
        assert len(self.real_output) == len(self.expected_output), (
            "Mismatch in number of real (%s) and expected (%s) strings"
            % (len(self.real_output), len(self.expected_output))
        )
        assert len(self.real_output) > 0
        assert isinstance(self.real_output[0], str), (
            "%r is not text type (not a unicode for Py2.x, not a str for Py3.x"
            % self.real_output[0]
        )
    ...
    def run_test(self, name, i):
        ...
        assert self.real_output[i] == self.expected_output[i], (
            "Real output %r doesn't match to expected %r for example #%s"
            % (self.real_output[i], self.expected_output[i], i)
        )
