...
def rl_ellipsis(x):
    ...
    patterns = (
        ...
        # словом нужно убрать
        (re.compile(r'(^|\"|\u201c|\xab)\s*\u2026\s*([А-Яа-яA-Za-z])', re.UNICODE), '\\1\u2026\\2'),
...
def rl_initials(x):
    ...
    """
    return re.sub(
        re.compile(r'([А-Я])\.\s*([А-Я])\.\s*([А-Я][а-я]+)', re.UNICODE),
        '\\1.\\2.\u2009\\3',
        x
    )
...
def rl_dashes(x):
    ...
    patterns = (
        # тире
        (re.compile(r'(^|(.\s))--?(([\s\u202f].)|$)', re.MULTILINE|re.UNICODE), '\\1\u2014\\3'),
...
def rl_marks(x):
    ...
    patterns = (
        ...
        (re.compile(r'\([cCсС]\)\s*(\d+)', re.UNICODE), '\u00a9\u202f\\1'),
        (r'([^+])(\+\-|\-\+)', '\\1\u00b1'), # ±
        # градусы с минусом
        (r'\-(\d+)[\s]*([FCС][^\w])', '\u2212\\1\202f\u00b0\\2'), # −12 °C, −53 °F
        # градусы без минуса
        (r'(\d+)[\s]*([FCС][^\w])', '\\1\u202f\u00b0\\2'), # 12 °C, 53 °F
        # ® и ™ приклеиваются к предыдущему слову, без пробела
        (re.compile(r'([A-Za-zА-Яа-я\!\?])\s*(\xae|\u2122)', re.UNICODE), '\\1\\2'),
        # No5 -> № 5
        (re.compile(r'(\s)(No|no|NO|\u2116)[\s\u2009]*(\d+)', re.UNICODE), '\\1\u2116\u2009\\3'),
    )
