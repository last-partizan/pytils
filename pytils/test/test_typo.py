...
class HelpersTestCase(unittest.TestCase):
    ...
    def testGetRuleByName(self):
        ...
        self.assertEqual("rl_testrule", typo._get_rule_by_name("testrule").__name__)  # type: ignore[unresolved-attribute]

    def testResolveRule(self):
        """
        unit-test for pytils.typo._resolve_rule
        """
        self.assertTrue(callable(typo._resolve_rule_name("testrule")[1]))
        self.assertTrue(callable(typo._resolve_rule_name(cb_testrule)[1]))
        self.assertEqual("testrule", typo._resolve_rule_name("testrule")[0])
        self.assertEqual("cb_testrule", typo._resolve_rule_name(cb_testrule)[0])

    def testResolveRuleWithForcedName(self):
        """
        unit-test for pytils.typo._resolve_rule with forced_name arg
        """
        self.assertTrue(callable(typo._resolve_rule_name("testrule", "newrule")[1]))
        self.assertTrue(callable(typo._resolve_rule_name(cb_testrule, "newrule")[1]))
        self.assertEqual("newrule", typo._resolve_rule_name("testrule", "newrule")[0])
        self.assertEqual("newrule", typo._resolve_rule_name(cb_testrule, "newrule")[0])
...
class TypographyApplierTestCase(unittest.TestCase):
    ...
    def testExpandEmptyArgs(self):
        self.assertEqual({}, typo.Typography().rules)
        self.assertEqual([], typo.Typography().rules_names)

    def testExpandSimpleStrArgs(self):
        self.assertEqual(
            {"testrule": typo.rl_testrule}, typo.Typography("testrule").rules
        )
        self.assertEqual(["testrule"], typo.Typography("testrule").rules_names)

    def testExpandDictStrArgs(self):
        self.assertEqual(
            {"testrule": typo.rl_testrule, "newrule": typo.rl_testrule},
    ...
    def testExpandSimpleCallableArgs(self):
        self.assertEqual(
            {"cb_testrule": cb_testrule}, typo.Typography(cb_testrule).rules
        )
        self.assertEqual(["cb_testrule"], typo.Typography(cb_testrule).rules_names)

    def testExpandDictCallableArgs(self):
        self.assertEqual(
            {"cb_testrule": cb_testrule, "newrule": cb_testrule},
    ...
    def testExpandMixedArgs(self):
        self.assertEqual(
            {"cb_testrule": cb_testrule, "newrule": typo.rl_testrule},
        ...
        self.assertEqual(
            {"cb_testrule": cb_testrule, "testrule": typo.rl_testrule},
    ...
    def testRecommendedArgsStyle(self):
        ...
        self.assertEqual(
            ...
            typo.Typography(
                [cb_testrule], ["testrule"], {"newrule": lambdarule}
            ).rules_names,
        )
...
class RulesTestCase(unittest.TestCase):
    def checkRule(self, name, input_value, expected_result):
        ...
        """
        self.assertEqual(expected_result, typo._get_rule_by_name(name)(input_value))
    ...
    def testEllipsis(self):
        ...
        self.checkRule("ellipsis", "...Дааааа", "…Дааааа")
        self.checkRule("ellipsis", "... Дааааа", "…Дааааа")
    ...
    def testMarks(self):
        ...
        self.checkRule("marks", "14 Foo", "14 Foo")
        self.checkRule("marks", "Coca-cola(tm)", "Coca-cola™")
        self.checkRule("marks", "(c) 2008 Юрий Юревич", "©\u202f2008 Юрий Юревич")
        self.checkRule("marks", "Microsoft (R) Windows (tm)", "Microsoft® Windows™")
        self.checkRule(
            "marks",
            "Школа-гимназия No 3",
            "Школа-гимназия №\u20093",
    ...
    def testQuotes(self):
        ...
        """
        self.checkRule("quotes", 'ООО "МСК "Аско-Забота"', "ООО «МСК «Аско-Забота»")
        self.checkRule(
            "quotes",
            'ООО\u202f"МСК\u202f"Аско-Забота"',
            "ООО\u202f«МСК\u202f«Аско-Забота»",
        ...
        self.checkRule(
            "quotes", "Двигатели 'Pratt&Whitney'", "Двигатели “Pratt&Whitney”"
        )
        self.checkRule(
            "quotes",
