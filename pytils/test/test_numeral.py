...
class ChoosePluralTestCase(unittest.TestCase):
    ...
    def checkChoosePlural(self, amount, estimated):
        ...
        """
        self.assertEqual(pytils.numeral.choose_plural(amount, self.variants),
                          estimated)
...
class GetFloatRemainderTestCase(unittest.TestCase):
    ...
    def testFloatRemainder(self):
        ...
        """
        self.assertEqual(pytils.numeral._get_float_remainder(1.3),
                          '3')
        self.assertEqual(pytils.numeral._get_float_remainder(2.35, 1),
                          '4')
        self.assertEqual(pytils.numeral._get_float_remainder(123.1234567891),
                          '123456789')
        self.assertEqual(pytils.numeral._get_float_remainder(2.353, 2),
                          '35')
        self.assertEqual(pytils.numeral._get_float_remainder(0.01),
                          '01')
        self.assertEqual(pytils.numeral._get_float_remainder(5),
                          '0')
    ...
    def testFloatRemainderDecimal(self):
        ...
        """
        D = decimal.Decimal
        self.assertEqual(pytils.numeral._get_float_remainder(D("1.3")),
                          '3')
        self.assertEqual(pytils.numeral._get_float_remainder(D("2.35"), 1),
                          '4')
        self.assertEqual(pytils.numeral._get_float_remainder(D("123.1234567891")),
                          '123456789')
        self.assertEqual(pytils.numeral._get_float_remainder(D("2.353"), 2),
                          '35')
        self.assertEqual(pytils.numeral._get_float_remainder(D("0.01")),
                          '01')
        self.assertEqual(pytils.numeral._get_float_remainder(D("5")),
                          '0')
...
class RublesTestCase(unittest.TestCase):
    ...
    def testRubles(self):
        ...
        """
        self.assertEqual(pytils.numeral.rubles(10.01),
                          "десять рублей одна копейка")
        self.assertEqual(pytils.numeral.rubles(10.10),
                          "десять рублей десять копеек")
        self.assertEqual(pytils.numeral.rubles(2.353),
                          "два рубля тридцать пять копеек")
        self.assertEqual(pytils.numeral.rubles(2.998),
                          "три рубля")
        self.assertEqual(pytils.numeral.rubles(3),
                          "три рубля")
        self.assertEqual(pytils.numeral.rubles(3, True),
                          "три рубля ноль копеек")
        self.assertEqual(pytils.numeral.rubles(long(3)),
                          "три рубля")
    ...
    def testRublesDecimal(self):
        ...
        """
        D = decimal.Decimal
        self.assertEqual(pytils.numeral.rubles(D("10.01")),
                          "десять рублей одна копейка")
        self.assertEqual(pytils.numeral.rubles(D("10.10")),
                          "десять рублей десять копеек")
        self.assertEqual(pytils.numeral.rubles(D("2.35")),
                          "два рубля тридцать пять копеек")
        self.assertEqual(pytils.numeral.rubles(D(3)),
                          "три рубля")
        self.assertEqual(pytils.numeral.rubles(D(3), True),
                          "три рубля ноль копеек")
...
class InWordsTestCase(unittest.TestCase):
    ...
    def testInt(self):
        ...
        self.assertEqual(pytils.numeral.in_words_int(3521),
                          "три тысячи пятьсот двадцать один")
        self.assertEqual(pytils.numeral.in_words_int(3500),
                          "три тысячи пятьсот")
        self.assertEqual(pytils.numeral.in_words_int(5231000),
                          "пять миллионов двести тридцать одна тысяча")
        self.assertEqual(pytils.numeral.in_words_int(long(10)), "десять")
    ...
    def testFloat(self):
        ...
        """
        self.assertEqual(pytils.numeral.in_words_float(10.0),
                          "десять целых ноль десятых")
        self.assertEqual(pytils.numeral.in_words_float(2.25),
                          "две целых двадцать пять сотых")
        self.assertEqual(pytils.numeral.in_words_float(0.01),
                          "ноль целых одна сотая")
        self.assertEqual(pytils.numeral.in_words_float(0.10),
    ...
    def testDecimal(self):
        ...
        """
        D = decimal.Decimal
        self.assertEqual(pytils.numeral.in_words_float(D("10.0")),
                          "десять целых ноль десятых")
        self.assertEqual(pytils.numeral.in_words_float(D("2.25")),
                          "две целых двадцать пять сотых")
        self.assertEqual(pytils.numeral.in_words_float(D("0.01")),
                          "ноль целых одна сотая")
        # поскольку это Decimal, то здесь нет незначащих нулей
        # т.е. нули определяют точность, поэтому десять сотых,
        # а не одна десятая
        self.assertEqual(pytils.numeral.in_words_float(D("0.10")),
    ...
    def testWithGenderOldStyle(self):
        ...
        """
        self.assertEqual(pytils.numeral.in_words(21, 1),
                          "двадцать один")
        self.assertEqual(pytils.numeral.in_words(21, 2),
                          "двадцать одна")
        self.assertEqual(pytils.numeral.in_words(21, 3),
                          "двадцать одно")
        # на дробные пол не должен влиять - всегда в женском роде
        self.assertEqual(pytils.numeral.in_words(21.0, 1),
                          "двадцать одна целая ноль десятых")
        self.assertEqual(pytils.numeral.in_words(21.0, 2),
                          "двадцать одна целая ноль десятых")
        self.assertEqual(pytils.numeral.in_words(21.0, 3),
                          "двадцать одна целая ноль десятых")
        self.assertEqual(pytils.numeral.in_words(long(21), 1),
                          "двадцать один")
    ...
    def testWithGender(self):
        ...
        Unit-test for in_words_float with gender (old-style, i.e. ints)
        """
        self.assertEqual(pytils.numeral.in_words(21, pytils.numeral.MALE),
                          "двадцать один")
        self.assertEqual(pytils.numeral.in_words(21, pytils.numeral.FEMALE),
                          "двадцать одна")
        self.assertEqual(pytils.numeral.in_words(21, pytils.numeral.NEUTER),
                          "двадцать одно")
        # на дробные пол не должен влиять - всегда в женском роде
        self.assertEqual(pytils.numeral.in_words(21.0, pytils.numeral.MALE),
                          "двадцать одна целая ноль десятых")
        self.assertEqual(pytils.numeral.in_words(21.0, pytils.numeral.FEMALE),
                          "двадцать одна целая ноль десятых")
        self.assertEqual(pytils.numeral.in_words(21.0, pytils.numeral.NEUTER),
                          "двадцать одна целая ноль десятых")
        self.assertEqual(pytils.numeral.in_words(long(21), pytils.numeral.MALE),
                          "двадцать один")
    ...
    def testCommon(self):
        ...
        self.assertEqual(pytils.numeral.in_words(3521),
                          "три тысячи пятьсот двадцать один")
        self.assertEqual(pytils.numeral.in_words(3500),
                          "три тысячи пятьсот")
        self.assertEqual(pytils.numeral.in_words(5231000),
                          "пять миллионов двести тридцать одна тысяча")
        self.assertEqual(pytils.numeral.in_words(10.0),
                          "десять целых ноль десятых")
        self.assertEqual(pytils.numeral.in_words(2.25),
                          "две целых двадцать пять сотых")
        self.assertEqual(pytils.numeral.in_words(0.01),
                          "ноль целых одна сотая")
        self.assertEqual(pytils.numeral.in_words(0.10),
                          "ноль целых одна десятая")
        self.assertEqual(pytils.numeral.in_words(long(10)), "десять")
        self.assertEqual(pytils.numeral.in_words(D("2.25")),
                          "две целых двадцать пять сотых")
        self.assertEqual(pytils.numeral.in_words(D("0.01")),
                          "ноль целых одна сотая")
        self.assertEqual(pytils.numeral.in_words(D("0.10")),
                          "ноль целых десять сотых")
...
class SumStringTestCase(unittest.TestCase):
    ...
    def ckMaleOldStyle(self, amount, estimated):
        ...
        """
        self.assertEqual(pytils.numeral.sum_string(amount,
                                                    1,
                                                    self.variants_male),
                          estimated)
    ...
    def testSumStringOldStyleGender(self):
        ...
        self.assertEqual("одиннадцать негритят",
