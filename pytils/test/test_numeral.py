...
class ChoosePluralTestCase(unittest.TestCase):
    ...
    def checkChoosePlural(self, amount, estimated):
        ...
        """
        self.assertEqual(pytils.numeral.choose_plural(amount, self.variants), estimated)
    ...
    def testChoosePluralExceptions(self):
        ...
        """
        self.assertRaises(ValueError, pytils.numeral.choose_plural, 25, "any,bene")
    ...
    def testChoosePluralVariantsInStr(self):
        ...
        """
        self.assertEqual(
            pytils.numeral.choose_plural(1, "гвоздь,гвоздя, гвоздей"), "гвоздь"
        )
        self.assertEqual(
...
class GetPluralTestCase(unittest.TestCase):
    ...
    def testGetPluralAbsence(self):
        ...
        self.assertEqual(
            pytils.numeral.get_plural(
                1, "комментарий, комментария, комментариев", "без комментариев"
            ),
            "1 комментарий",
        )
        self.assertEqual(
            pytils.numeral.get_plural(
                0, "комментарий, комментария, комментариев", "без комментариев"
            ),
            "без комментариев",
        )
    ...
    def testGetPluralLegacy(self):
        ...
        """
        self.assertEqual(
            pytils.numeral._get_plural_legacy(
                1, "комментарий, комментария, комментариев"
            ),
            "1 комментарий",
        )
        self.assertEqual(
            pytils.numeral._get_plural_legacy(
                0, "комментарий, комментария, комментариев"
            ),
            "0 комментариев",
        )
        self.assertEqual(
...
class GetFloatRemainderTestCase(unittest.TestCase):
    ...
    def testFloatRemainder(self):
        ...
        """
        self.assertEqual(pytils.numeral._get_float_remainder(1.3), "3")
        self.assertEqual(pytils.numeral._get_float_remainder(2.35, 1), "4")
        self.assertEqual(
            pytils.numeral._get_float_remainder(123.1234567891), "123456789"
        )
        self.assertEqual(pytils.numeral._get_float_remainder(2.353, 2), "35")
        self.assertEqual(pytils.numeral._get_float_remainder(0.01), "01")
        self.assertEqual(pytils.numeral._get_float_remainder(5), "0")
    ...
    def testFloatRemainderDecimal(self):
        ...
        """
        D = decimal.Decimal
        self.assertEqual(pytils.numeral._get_float_remainder(D("1.3")), "3")
        self.assertEqual(pytils.numeral._get_float_remainder(D("2.35"), 1), "4")
        self.assertEqual(
            pytils.numeral._get_float_remainder(D("123.1234567891")), "123456789"
        )
        self.assertEqual(pytils.numeral._get_float_remainder(D("2.353"), 2), "35")
        self.assertEqual(pytils.numeral._get_float_remainder(D("0.01")), "01")
        self.assertEqual(pytils.numeral._get_float_remainder(D("5")), "0")
...
class RublesTestCase(unittest.TestCase):
    ...
    def testRubles(self):
        ...
        """
        self.assertEqual(pytils.numeral.rubles(10.01), "десять рублей одна копейка")
        self.assertEqual(pytils.numeral.rubles(10.10), "десять рублей десять копеек")
        self.assertEqual(pytils.numeral.rubles(2.353), "два рубля тридцать пять копеек")
        self.assertEqual(pytils.numeral.rubles(2.998), "три рубля")
        self.assertEqual(pytils.numeral.rubles(3), "три рубля")
        self.assertEqual(pytils.numeral.rubles(3, True), "три рубля ноль копеек")
    ...
    def testRublesDecimal(self):
        ...
        D = decimal.Decimal
        self.assertEqual(
            pytils.numeral.rubles(D("10.01")), "десять рублей одна копейка"
        )
        self.assertEqual(
            pytils.numeral.rubles(D("10.10")), "десять рублей десять копеек"
        )
        self.assertEqual(
            pytils.numeral.rubles(D("2.35")), "два рубля тридцать пять копеек"
        )
        self.assertEqual(pytils.numeral.rubles(D(3)), "три рубля")
        self.assertEqual(pytils.numeral.rubles(D(3), True), "три рубля ноль копеек")
...
class InWordsTestCase(unittest.TestCase):
    ...
    def testInt(self):
        ...
        self.assertEqual(
            pytils.numeral.in_words_int(3521), "три тысячи пятьсот двадцать один"
        )
        self.assertEqual(pytils.numeral.in_words_int(3500), "три тысячи пятьсот")
        self.assertEqual(
            pytils.numeral.in_words_int(5231000),
    ...
    def testFloat(self):
        ...
        """
        self.assertEqual(
            pytils.numeral.in_words_float(10.0), "десять целых ноль десятых"
        )
        self.assertEqual(
            pytils.numeral.in_words_float(2.25), "две целых двадцать пять сотых"
        )
    ...
    def testDecimal(self):
        ...
        D = decimal.Decimal
        self.assertEqual(
            pytils.numeral.in_words_float(D("10.0")), "десять целых ноль десятых"
        )
        self.assertEqual(
            pytils.numeral.in_words_float(D("2.25")), "две целых двадцать пять сотых"
        )
        self.assertEqual(
        ...
        # а не одна десятая
        self.assertEqual(
            pytils.numeral.in_words_float(D("0.10")), "ноль целых десять сотых"
        )
    ...
    def testWithGenderOldStyle(self):
        ...
        """
        self.assertEqual(
            pytils.numeral.in_words(21, pytils.numeral.Gender.MALE), "двадцать один"
        )
        self.assertEqual(
            pytils.numeral.in_words(21, pytils.numeral.Gender.FEMALE), "двадцать одна"
        )
        self.assertEqual(
            pytils.numeral.in_words(21, pytils.numeral.Gender.NEUTER), "двадцать одно"
        )
        # на дробные пол не должен влиять - всегда в женском роде
        self.assertEqual(
            pytils.numeral.in_words(21.0, 1), "двадцать одна целая ноль десятых"
        )
        self.assertEqual(
            pytils.numeral.in_words(21.0, 2), "двадцать одна целая ноль десятых"
        )
        self.assertEqual(
            pytils.numeral.in_words(21.0, 3), "двадцать одна целая ноль десятых"
        )
    ...
    def testWithGender(self):
        ...
        # на дробные пол не должен влиять - всегда в женском роде
        self.assertEqual(
            pytils.numeral.in_words(21.0, pytils.numeral.Gender.MALE),
            "двадцать одна целая ноль десятых",
        )
        self.assertEqual(
            pytils.numeral.in_words(21.0, pytils.numeral.Gender.FEMALE),
            "двадцать одна целая ноль десятых",
        )
        self.assertEqual(
            pytils.numeral.in_words(21.0, pytils.numeral.Gender.NEUTER),
            "двадцать одна целая ноль десятых",
        )
    ...
    def testCommon(self):
        ...
        """
        D = decimal.Decimal
        self.assertEqual(pytils.numeral.in_words(10), "десять")
        self.assertEqual(pytils.numeral.in_words(5), "пять")
        self.assertEqual(pytils.numeral.in_words(102), "сто два")
        self.assertEqual(
            pytils.numeral.in_words(3521), "три тысячи пятьсот двадцать один"
        )
        self.assertEqual(pytils.numeral.in_words(3500), "три тысячи пятьсот")
        self.assertEqual(
            pytils.numeral.in_words(5231000),
            "пять миллионов двести тридцать одна тысяча",
        )
        self.assertEqual(pytils.numeral.in_words(10.0), "десять целых ноль десятых")
        self.assertEqual(pytils.numeral.in_words(2.25), "две целых двадцать пять сотых")
        self.assertEqual(pytils.numeral.in_words(0.01), "ноль целых одна сотая")
        self.assertEqual(pytils.numeral.in_words(0.10), "ноль целых одна десятая")
        self.assertEqual(
            pytils.numeral.in_words(D("2.25")), "две целых двадцать пять сотых"
        )
        self.assertEqual(pytils.numeral.in_words(D("0.01")), "ноль целых одна сотая")
        self.assertEqual(pytils.numeral.in_words(D("0.10")), "ноль целых десять сотых")
        self.assertEqual(pytils.numeral.in_words(D("10")), "десять")
...
class SumStringTestCase(unittest.TestCase):
    ...
    def ckMaleOldStyle(self, amount, estimated):
        ...
        self.assertEqual(
            pytils.numeral.sum_string(
                amount, pytils.numeral.Gender.MALE, self.variants_male
            ),
            estimated,
        )
    ...
    def ckFemaleOldStyle(self, amount, estimated):
        ...
        self.assertEqual(
            pytils.numeral.sum_string(
                amount, pytils.numeral.Gender.FEMALE, self.variants_female
            ),
            estimated,
        )
    ...
    def testSumStringOldStyleGender(self):
        ...
        self.assertEqual(
            ...
            pytils.numeral.sum_string(
                11, pytils.numeral.Gender.MALE, "негритенок,негритенка,негритят"
            ),
        )
