...
class TemplateTagsCommonsTestCase(TestCase):
    def testInitDefaults(self):
        ...
        self.assertEqual(
            tt.init_defaults(debug=False, show_value=True), ("%(value)s", "%(value)s")
        )
        self.assertEqual(
            tt.init_defaults(debug=True, show_value=False),
            ("unknown: %(error)s", "unknown: %(error)s"),
        )
        self.assertEqual(
            tt.init_defaults(debug=True, show_value=True),
            ("unknown: %(error)s", "unknown: %(error)s"),
        )
